...
- A compile-time commissioning lock defaults to torque disabled. It must remain disabled until the blocking evidence in [VCU safety-remediation status](SAFETY_REMEDIATION_STATUS.md) is closed and independently reviewed.
...
- Simultaneous brake application and APPS above 25 percent latches a BPPS fault immediately; it remains faulted until APPS falls below 5 percent. Dual brake-sensor disagreement uses a separate 50 ms persistence check.
- CAN is initialized for 500 kbit/s with FIFO0 reception, filtering, and bus-off notification.
- The serial interface includes the diagnostic, pedal-response, configuration, service-mode, and reset commands documented in [VCU Operator Guide](VCU_OPERATOR_GUIDE.md). Runtime task suspend/resume is disabled in the vehicle build.
- CAN identifiers, ranges, and expected message lengths are defined in `Core/Inc/Peripherals/can_protocol.h` and referenced by the CAN receiver, motor-controller command builder, and CLI.
...
3. The RTD sound must be produced by the approved buzzer/dashboard path. `rtd_sound_active` is only an internal firmware request signal in this revision; no verified output driver consumes it. Do not enable tractive torque until that path is implemented and the audible timing and sound are verified.
...
4. Verify APPS above 25 percent while braking immediately produces a latched BPPS fault and that it clears only below 5 percent APPS. Independently verify dual brake-sensor disagreement timing.
