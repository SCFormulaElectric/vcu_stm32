...
void can_transmitter_task(void *argument)
{
    ...
    for (;;)
    {
        ...
        while (xQueueReceive(can_bus->can_tx_queue, &can_tx, 0) == pdPASS) {
            ...
            if (0 == mailbox_found) {
                ...
                serial_log("Could not find a mailbox for can message");
                break;
            }
            ...
            if (HAL_CAN_AddTxMessage(can_bus->hcan, &tx_header, tx_data, &txMailbox) != HAL_OK) {
                serial_log("Error sending can message, id: %d", tx_header.StdId);
            }
        }
